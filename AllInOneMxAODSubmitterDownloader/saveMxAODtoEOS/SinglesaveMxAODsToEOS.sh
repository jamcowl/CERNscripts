...
# and copy them to the speicified directories in EOS.  
...
#checkProxy=$(voms-proxy-info |& grep "Proxy not found") # this doesn't work well...
#[ ! -z "$checkProxy" ] && echo "8Twmp224tony" | voms-proxy-init -voms atlas
...
[ ! -z "$Output" ]    && echo $datasetName >>output/failedDatasetsDownload.txt && echo "FAILED $datasetName" && return 1
